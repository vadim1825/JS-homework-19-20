...
$(function() {
  ...
  //make an array which can be accessed by
  ...
  //get an array of skills
    ...
    $skills = _.union($skills); //remove duplicates

  //sorted alphabetically

    var $newSkill = $skills.sort(function(Str1, Str2) {
  ...
  //sort the main array to get a sorted array of names

    var $newData = $data.sort(function(val1, val2) {
        var val1_1 = val1.friends.length;
        var val2_1 = val2.friends.length;
        if (val1_1>val2_1) return 1;
        if (val1_1<val2_1) return -1;
        return 0;
    });

  //get the array of names sorted by number of friends

    var $names = _.map($data, 'name');

  //get the array of friends

    var $friends = _.map($data, 'friends'); //get objects by key friends

    $friends = _.flatten($friends);

    $friends = _.map($friends, 'name'); //get the names of friends

    $friends = _.union($friends); //remove duplicates

  //create an object with arrays of results

    var $tData = new Object();

    $tData.skills = $newSkill;

    $tData.names = $names;

    $tData.friends = $friends;
